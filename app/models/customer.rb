...
class Customer < ActiveRecord::Base
  ...
  def shares
    Rails.cache.fetch("customer_shares_#{self.id}", expires_in: 10.minutes) do
      s = PhotoSession.where(event_id: self.events.ids).pluck(:twitter_shares, :facebook_shares, :instagram_shares, :custom_button_shares, :downloads).transpose.map {|a| a.inject(:+)}
